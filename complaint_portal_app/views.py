...
def OE_login(request):
    if request.method == "POST":
        ...
        # print("user_obj", user_obj.values())
        ...
        if not user_obj.exists():
            messages.warning(request, "Invalid credentials bhai!")
            return HttpResponseRedirect(request.path_info)
        user=User.objects.get(username=username)
        print("user by get: ",user)

        user_obj = authenticate(username=username, password=password)
        print("authenticate ho gaya user: ", user_obj)
        if user_obj:
            if (user_belongs_to_admin_group(user)):
                print("logging!!!")
                login(request, user)
                print("logged", request.user)

                return render(request, 'OE_dashboard.html', {"user": user})
            else:
                messages.warning(request, "Invalid credentials!")
                print("user is not from admin group")
                return redirect('OE_login')
